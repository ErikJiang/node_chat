app.factory('server', ['$cacheFactory', '$q', '$http', 'socket', function($cacheFactory, $q, $http, socket) {
  var cache = window.cache = $cacheFactory('nodechat')
  socket.on('nodechat', function(data) {
    switch (data.action) {
      case 'getRoom':
        if (data._roomId) {
